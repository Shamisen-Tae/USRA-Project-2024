...
r_cell = np.array([[30,30,-30,-30,10,10],[30,-30,-30,30,10,-10]])
r_cell_plot = r_cell
...
direction_vector = np.array([[-1, 0], [0, -1], [1,0], [0,1]])

#boundary is x = 60, y = 60, x = -60, y = -60 resptively

def repel_strength(d):
    return max(sigma_rw-d,0)*exp(1.0/(d+0.003))
...
def repel_force(point):
    ...
    result += repel_strength(x0 - point[0]) * direction_vector[0] +\
            repel_strength(point[0] + x0) * direction_vector[2] +\
            repel_strength(y0 - point[1]) * direction_vector[1] +\
            repel_strength(point[1] + y0) * direction_vector[3]
    return result
...
while n<num_steps:
    ...
    for i in range(r_cell.shape[1]):
        ...
        if not -x0 <= c1[0] <= x0 or not -y0 <= c1[1] <= y0:
            continue
        ...
        for j in range(points[i].shape[1]):
            if max(u_TGbeta.vector().get_local())>1E-7 and all(np.linalg.norm(points[i],axis=0)>2) and recover_flag[i]:
                ...
                #     np.array([lambda_curr,lambda_curr]) * dt +\
                #           alpha*tau[j]*dt
                points_0[i][:,j] = points[i][:, j] + rw + alpha*tau[i][j]*dt +\
